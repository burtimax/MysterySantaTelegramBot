...
{
    public class HideParticipantCommand : BotCommand
    {
        public HideParticipantCommand(string str) : base(str)
        {
        ...
        public override async Task<Hop> ExecCommand(UpdateBotModel data)
        {
            ...
            using (SantaContext _db = new SantaContext())
            {
                ...
                userToHideInfo.Invisible = true;
                await _db.Repos.UserInfo.UpdateAsync(userToHideInfo);
                await data.bot.SendTextMessageAsync(AppConstants.SupportUserId, $"Пользователь с Id = {userHideId} спрятан в показах");
            }
